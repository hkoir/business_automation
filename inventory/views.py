            ...
            with transaction.atomic():
                ...
                for item in transfer_basket:
                    ...
                    if not created:
                        ...
                        raise ValueError(f"No stock exists in {source_warehouse.name} for {product.name}. Cannot transfer.")
...
def inventory_aggregate_list(request):
    ...
    if form.is_valid():
        ...
        # Apply filters only if a product is selected or date range is provided
        if product_name:
    ...
    # Only generate chart data if aggregated_data exists
    if aggregated_data:
        chart_data = {
