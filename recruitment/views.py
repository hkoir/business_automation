...
def applicant_list(request, job_id):   
    ...
    candidates = job_instance.candidates.all()
...
#     create_instance = get_object_or_404(Job, id=id)
#     message_text = "updated successfully!" if id else "added successfully!"  
...
#         form_intance.save()        
#         messages.success(request, message_text)
#         return redirect('recruitment:create_exam',id=create_instance.id)   
...
#     instance = get_object_or_404(Exam, id=id)
#     message_text = "Question added successfully!"  

#     if request.method == 'POST':
#         form = QuestionForm(request.POST)
#         if form.is_valid():
#             form_instance = form.save(commit=False)
#             form_instance.exam = instance  
#             form_instance.user = request.user
#             form_instance.save()
#             messages.success(request, message_text)
#             return redirect('recruitment:create_questions', id=id)
#     else:
#         form = QuestionForm()

#     questions = instance.questions.all().order_by('-created_at')
#     paginator = Paginator(questions, 5)
#     page_number = request.GET.get('page')
#     page_obj = paginator.get_page(page_number)

#     return render(request, 'recruitment/create_questions.html', {
#         'form': form,
#         'instance': instance,       
#         'page_obj': page_obj
#     })
...
def take_exam(request, exam_id, candidate_id):
    ...
    return render(request, 'recruitment/take_exam.html', {
        'exam': exam,
        'candidate': candidate,
        'form': form,
        'exam_start_time': localtime(exam.start_time).isoformat(),
...
def panel_member_scoring(request, job_id, exam_id, candidate_id, panel_member_id):
    ...
    panel_member = get_object_or_404(PanelMember, id=panel_member_id)

    # if InterviewScore.objects.filter(interview__candidate=candidate, panel_member=panel_member).exists():
    #     messages.error(request, "You have  already interviewed this candidate.")
    #     return redirect('recruitment:job_exam_list', job.id)
    ...
    panel_members = PanelMember.objects.all()  
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            average_interview_score = float(total_interview_score) / 5.0
            ...
            candidate_interview_scores =  candidate_total_score / (num_panel_members * 5) if num_panel_members > 0 else 0
...
def candidate_interview_scores(request, candidate_id):
    ...
    total_average_score= (float(cv_screening_score) + float(exam_score) + float(average_interview_score)) / 3.0
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            total = exam_screening_result.aggregate(
                candidate_pass=Count('id', filter=Q(exam_status='EXAM-PASS')),
                candidate_fail=Count('id', filter=Q(exam_status='EXAM-FAIL'))
            )
...
def selected_candidate(request):
    ...
    if request.method == 'POST' and form_name == 'form-1':        
        ...
        if form.is_valid():
            ...
            if job_title:
                candidates = Candidate.objects.filter(applied_job__title = job_title).order_by('-total_score')
                total_candidates = candidates.count()
                total_selected_candidates = candidates.filter(status='SELECTED').count()
...
def send_offer_letters_to_top_scorers(request):
    employee = Employee.objects.filter(user_profile__user=request.user).first()
    selected_candidates = Candidate.objects.filter(status='SELECTED').order_by('-total_score')
